...
import unittest
from requesttest import registerunit
from requesttest import loginunit
import time
...
def send_mail(new_report):
    ...
    smtp.login('1377830227@qq.com', 'vakrxjqxoxeojcja')  # 不是你的邮箱登录密码，而是你登录成功之后设置授权码
    smtp.sendmail('1377830227@qq.com', '1377830227@qq.com', msg.as_string())
    smtp.quit()
    print ('邮件已发出！注意查收。')
...
if __name__=='__main__':
    ...
    suit.addTest(unittest.makeSuite(registerunit.Register))
    suit.addTest(unittest.makeSuite(loginunit.Login))
    #直接添加单元测试到测试套件中
